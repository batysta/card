...
// Cole aqui a URL pública JSON da sua planilha Google Sheets.
...
async function loadNews() {
  try {
    ...
    // Ponto de injeção dos dados da planilha:
    // Espera um array JSON com as colunas:
    // id, titulo, data, resumo, conteudo, imagem_url, link, destaque, status
    const response = await fetch(SHEETS_JSON_URL, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Falha HTTP ${response.status}`);

    const rawData = await response.json();
    const mappedNews = Array.isArray(rawData) ? prepareNewsRows(rawData) : [];
    const safeNews = mappedNews.length ? mappedNews : fallbackNews;
    ...
    const highlighted =
      safeNewsWithKeys.find((item) => item.destaque === 'SIM') || safeNewsWithKeys[0];
